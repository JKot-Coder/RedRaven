...
namespace RR::Ecs
{
    ...
    void World::OrderSystems()
    {
        ...
        auto insertOrderEdge = [&](const SystemHandle& system, ComponentId require) {
            ...
            {
                ECS_ENSURE(false, "ES <{}> is require token that is never produced by any system.", system.hashName.string.c_str());
                return;
            }
        ...
        auto loopDetected = [&](size_t idx, auto&) {
            ECS_ENSURE(false, "ES <{}> in graph to become cyclic and was removed from sorting. ES order is non-deterministic.", tmpSystemList[idx].hashName.string.c_str());
        };
