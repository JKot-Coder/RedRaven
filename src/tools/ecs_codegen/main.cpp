...
namespace RR
{
    Common::RResult main(int argc, char** argv, const std::shared_ptr<ParseTools::BufferWriter>& sinkOutput)
    {
        ...
        try
        {
            ...
            result = includeSystem->FindFile(sourcePath, "", pathInfo);

            if (RR_FAILED(result))
            {
                std::cerr << "Error: Could not find file " << sourcePath << " Error: " << Common::GetErrorMessage(result) << std::endl;
                return result;
            }

            std::shared_ptr<ParseTools::SourceFile> sourceFile;
            result = sourceManager->LoadFile(pathInfo, sourceFile);
            if (RR_FAILED(result))
            {
                std::cerr << "Error: Could not load file: " << sourcePath << " Error: " << Common::GetErrorMessage(result) << std::endl;
                return result;
            }
            ...
            if (RR_FAILED(result))
            {
                std::cerr << "Error: Could not parse file: " << sourcePath << " Error: " << Common::GetErrorMessage(result) << std::endl;
                return result;
            }
