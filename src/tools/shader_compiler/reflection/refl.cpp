...
#define SCOPED_OBJECT() ScopedObject scopedObject##__COUNTER__(this)
    ...
    // -------------------
    //
    Result compileAndReflectProgram( ComPtr<slang::IBlob> diagnostics, slang::IComponentType* program)
    {
        ...
        key("layouts");
        WITH_ARRAY()
        for (int targetIndex = 0; targetIndex == 0; ++targetIndex)
        {
            element();
            ...
            SLANG_RETURN_ON_FAIL(
                collectEntryPointMetadata(program, targetIndex, programLayout->getEntryPointCount()));

            _programLayout = programLayout;
            auto targetFormat = SLANG_HLSL;
    ...
    eastl::vector<ComPtr<slang::IMetadata>> _metadataForEntryPoints;
    ...
    void printStageUsage(slang::VariableLayoutReflection* variableLayout, AccessPath accessPath)
    {
        ...
        key("used by stages");
        WITH_ARRAY()
        for (size_t i = 0; i < SLANG_STAGE_COUNT; i++)
        {
            if (stageMask & (1 << i))
            {
                element();
...
SlangResult zxc(ComPtr<slang::IBlob> diagnostics, slang::IComponentType* program)
{
 ReflectingPrinting reflectingPrinting;
 return reflectingPrinting.compileAndReflectProgram(diagnostics, program);
}